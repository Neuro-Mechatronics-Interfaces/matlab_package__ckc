function pre_process_2x_4mm(input_root, block, options)
%PRE_PROCESS_2x_4mm  For pre-processing sets of 4x poly5 recordings, using 8x8 standard PCB grids and acquired using polybench, so that they are ready for the CKC reader.
%
% Syntax:
%   ckc.pre_process_2x_4mm(input_root, block, 'Name', value, ...);
%
% Inputs:
...
A = dir(fullfile(input_root, sprintf("%s_A*_%d.poly5", TANK, block)));
B = dir(fullfile(input_root, sprintf("%s_B*_%d.poly5", TANK, block)));
tabletFile = string(fullfile(input_root, sprintf("%s_TABLET_%d.bin", TANK, block)));
...
[data,~,ch_name] = io.load_align_saga_data_many(poly5_files, ...
    ...
    'ManualSyncIndex', 1, ...
    'IsTextile64', false);
[iUni,~,~] = ckc.get_saga_channel_masks(ch_name,...
    'ReturnNumericIndices',true);
uni = data.samples(iUni,:);
sample_rate = data.sample_rate;
sync = data.tablet.Pressure';

aux = [data.tablet.X'; data.tablet.Y'];
output_filename = fullfile(input_root, sprintf("%s_%s_%d.mat", TANK, options.OutputFileTag, block));
