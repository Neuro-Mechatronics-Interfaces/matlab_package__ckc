function data = template__reader(filepath,filename,epoch_length,options)
%TEMPLATE__READER Template for reading surface EMG acquired by TMSi-SAGA and arbitrary textile (monopolar 32-channel) surface array.
% 
% SYNTAX:
%   data = ckc.template__reader(filepath, filename, epoch_length, 'Name', value, ...);
%
% INPUTS:
...
if isempty(options.AUXchannels)
    ...
    elseif isfield(f,'aux')
        data.AUXchannels = f.aux;
    else
        data.AUXchannels = [];
    end
else
