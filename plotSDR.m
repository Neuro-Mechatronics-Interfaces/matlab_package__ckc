function [cum_IDR ,cum_EpochTime]=plotSDR(cMUPulses,MU_ID, fsamp,Force,BackColor,FrontColor)

MyColor=colormap(jet(256));
MyColor=MyColor(end : -floor(size(MyColor,1)/max([length(cMUPulses),1])) :1,:);
    ...
    if ~isempty(cMUPulses{k1})
        ...
        for k2 = 1:length(tmpEpochs)-1
            ...
            elseif length(tmpEpochTime) > 10 % filter epochs with more than 7 firings only
                ...
                tmpIDR = 1./tmpIPI;
                tmpEpochTime = [tmpEpochTime(1):1/fsamp2:tmpEpochTime(end)];
            elseif ~isempty(tmpEpochTime) % for shorter epochs use the avarage IDR value
                tmpIDR = [];
...
for k1=1:length(cum_IDR)
    ...
    if ~isempty(cum_EpochTime{k1})
        ht = text(cum_EpochTime{k1}{k2}(end)+0.1,cum_IDR{k1}{k2}(end),MU_ID{k1});
        set(ht,'Color',FrontColor);
        set(ht,'FontSize',10);
    end
end
